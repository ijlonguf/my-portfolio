...
export const GoogleGeminiEffect: React.FC<GoogleGeminiEffectProps> = ({ animate, className }) => {
  const textTopControls = useAnimation();
  const textBottomControls = useAnimation();

  useEffect(() => {
    const animationSequence = async () => {
      // Start Gemini animation
      animate.start({ pathLength: 1, transition: { duration: 2.5 } });
      ...
      const textAnimation = {
        backgroundPosition: ["100% 0%", "-400% 0%"],
        transition: {
          duration: 0.5,
          repeat: Infinity,
      ...
      textTopControls.start(textAnimation);
      textBottomControls.start(textAnimation);
    };

    animationSequence();
  }, [animate, textTopControls, textBottomControls]);
